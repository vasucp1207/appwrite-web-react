...
export function ForgotPassword({ theme, onChange, account }: {
  theme: string,
  onChange: () => void,
  account: any
}) {
  ...
  const forgotPassword = async () => {
    ...
    const res = await useCreateRecovery(email, account);
    if (res) console.log('✅ Recovery email sent.');
  ...
  return (
    <div className={`relative flex justify-center items-center w-[100vw] h-[100vh] ${theme === 'dark' ? 'bg-[#14141f] text-white' : 'bg-[#fafaff] text-[#373b4d]'}`}>
      ...
      <div className='w-96'>
        ...
        <input
          value={email}
          className={`w-full h-10 p-1 pl-[12px] pr-[12px] text-sm rounded border-[1px] border-[#e9eaf1] focus:outline-none focus:border-[#c5c7d7]`} type='text' placeholder='Email'
          onChange={setMail}
        />
